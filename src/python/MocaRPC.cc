...
initmocarpc(void)
{
  ...
  PyObject *mod;
  if ((mod = Py_InitModule3("mocarpc", methods, "MocaRPC module.")) == NULL) {
    return;
  }
...
static int handleRPCError(int32_t st, const char *message = NULL)
{
  ...
  if (error != NULL || MOCA_RPC_FAILED(st)) {
    char buff[32];
...
static int32_t withRetry(int32_t retry, int64_t deadline, Callable callback, RPCOpaqueData userData)
{
  int32_t st;
retry:
  if (MOCA_RPC_FAILED(st = callback(userData))) {
    if (--retry >= 0) {
      goto retry;
    }
    return st;
...
static int packetSetHeaders(PyPacket *self, PyObject *value, void *closure)
{
  ...
  KeyValuePairs<StringLite, StringLite> headers;
  MOCA_RPC_DO(convert(value, &headers));
...
static int packetSetPayload(PyPacket *self, PyObject *value, void *closure)
{
  ...
  StringLite payload;
  MOCA_RPC_DO(convert(value, &payload));
...
{
  int32_t st;
  if (MOCA_RPC_FAILED(st = self->channel->localId(&self->localId)) ||
      MOCA_RPC_FAILED(st = self->channel->localAddress(&self->localAddress, &self->localPort)) ||
      MOCA_RPC_FAILED(st = self->channel->remoteAddress(&self->remoteAddress, &self->remotePort)) ||
      MOCA_RPC_FAILED(st = self->channel->remoteId(&self->remoteId))) {
...
static int32_t channelEasyDoConnect(PyChannelEasy *self)
{
  ...
  if (self->channel == NULL || MOCA_RPC_FAILED(channelEasyAfterEstablished(self))) {
...
channelEasyResponseImpl(PyChannelEasy *self, int64_t id, int32_t code, PyObject *headers, Py_buffer *payload)
{
  ...
  int32_t st;
  if (MOCA_RPC_FAILED(st = withRetry(self->retry, self->timeout, callable(channelEasyDoResponse), self))) {
    channelEasyHandleRPCError(self, st);
    return NULL;
  } else {
...
{
  if (self->channel == NULL) {
    return handleRPCError(MOCA_RPC_DISCONNECTED, "Channel is not connected yet");
  } else {
    return RPC_OK;
  }
}
...
{
  if (MOCA_RPC_FAILED(channelEasyCheckConnected(self))) {
    return NULL;
  }
